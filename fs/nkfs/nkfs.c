...
#define SIMPLEFS_MAGIC 0x4E4B4653
...
#define SIMPLEFS_DISK_INODE_PER_BLOCK \
	(SIMPLEFS_BLOCKSIZE / SIMPLEFS_DISK_INODE_SIZE)
...
static struct simplefs_disk_inode *
simplefs_get_inode_disk(struct super_block *sb, unsigned long ino,
...
static struct dentry *simplefs_mkdir(struct mnt_idmap *idmap, struct inode *dir,
...
static const struct inode_operations simplefs_file_inode_ops = { NULL };
...
static int __init simplefs_module_init(void)
{
	...
	if ((simplefs_inode_cachep = kmem_cache_create(
		     "simplefs_inode_cache", sizeof(struct simplefs_inode), 0,
...
static int simplefs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if ((sfs_sb = kzalloc(sizeof(struct simplefs_super_block),
			      GFP_KERNEL)) == NULL) {
	...
	if ((sfs_sb->inode_bitmap_bh =
		     sb_bread(sb, SIMPLEFS_INODE_BITMAP_LOCATION)) == NULL) {
	...
	if ((sfs_sb->data_bitmap_bh =
		     sb_bread(sb, SIMPLEFS_DATA_BITMAP_LOCATION)) == NULL) {
...
static struct inode *simplefs_alloc_inode(struct super_block *sb)
{
	...
	if ((inode_sfs = kmem_cache_alloc(simplefs_inode_cachep, GFP_KERNEL)) ==
	    NULL)
		return NULL;
...
static int simplefs_iterate_shared(struct file *file, struct dir_context *ctx)
{
	...
	for (i = ctx->pos; i < SIMPLEFS_MAX_DENTRIES; ++i) {
		...
		if (!dir_emit(ctx, de[i].name, strlen(de[i].name), ino,
			      DT_UNKNOWN)) {
			brelse(bh);
			return 0;
		}
...
static int simplefs_alloc_data_block(struct super_block *sb)
{
	...
	if (bit >= max_data_blocks) {
		spin_unlock(&sfs_sb->lock);
		printk(KERN_ERR
		       "simplfs: No free data block found in bitmap\n");
		return -ENOSPC;
	}
...
static void simplefs_free_data_block(struct super_block *sb, int block)
{
	...
	if (block < sfs_sbd->data_block_start ||
	    block >= sfs_sbd->block_count) {
			      ...
			      struct inode *inode)
{
	...
	for (i = 0; i < SIMPLEFS_MAX_DENTRIES; ++i) {
		if (entry[i].inode == 0) {
			...
			strncpy(entry[i].name, dentry->d_name.name,
				SIMPLEFS_FILENAME_MAXLEN);
